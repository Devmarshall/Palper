...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Website
      .findOne(_.pick(params, _.keys(Website.schema.paths)))
      .populate(populate);
  },

  /**
   * Promise to count websites.
   *
   * @return {Promise}
   */
  ...
  add: async (values) => {
    ...
    const entry = await Website.create(data);

    // Create relational data and return the entry.
    return Website.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Website.associations.map(a => a.alias));
    const data = _.omit(values, Website.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Website.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Website.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Website.attributes).reduce((acc, curr) => {
      switch (Website.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
